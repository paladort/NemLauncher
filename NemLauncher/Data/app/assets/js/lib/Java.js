...
class Java {
  ...
  async getManifestJSON(){
    if(process.platform == "linux")
      return await fetch("http://download.paladium-pvp.fr/launcher/java/linux-manifest.json").then(res => res.json());
  ...
  async getBundle(){
    ...
    for(let [path, info] of manifest){
      ...
      let file = {};
      file.path = `${localStorage.getItem(".paladium")}/runtime/java/${path.replace(toDelete, "")}`;
